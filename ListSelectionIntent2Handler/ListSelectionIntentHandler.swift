...
extension IntentHandler: ListSelectionConfigurationIntentHandling {
    ...
    func provideShoppingListOptionsCollection(for intent: ListSelectionConfigurationIntent) async throws -> INObjectCollection<IntentShoppingList> {
        let results: [IntentShoppingList] = fetchActiveLists()
        let collection = INObjectCollection(items: results)
        return collection
    }
    
    func defaultShoppingList(for intent: ListSelectionConfigurationIntent) -> IntentShoppingList? {
